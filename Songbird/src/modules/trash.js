...
function initNodesAnswer(answer) {
  return {
    ...
    // wrap: answer.querySelector('.player'),
...
function initNodesDefault(main, answer) {
  const target = main.querySelector('.player');
  const { audio, name, id } = answer;
  const player = new AudioComponent(audio, name, id, target);
  return {
    id: null,
    // audio: main.querySelector('.audio'),
    // wrap: main.querySelector('.player'),
    // wrap: player,
...
function insertCorrectAnswers(elems, answer) {
  ...
  for (const el in elems) {
    if (el === 'image') {
      ...
      const { audio, name, id } = answer;
      new AudioComponent(audio, name, id, elems[el]);
      ...
      try {
        ...
        console.log(elems, 'ELEMS', 'INNERHTML');
        console.log(elems[el], 'ELEMS[EL]', 'INNERHTML');
        console.log(answer[el], 'ANSWER[EL]', 'INNERHTML');
      }
    }
  }
}
...
function setGuessBird(answer) {
  // console.log(nodesData.elemsDefault);
...
function startGame() {
  ...
  const elemsDefault = initNodesDefault(mainNode, answer);
